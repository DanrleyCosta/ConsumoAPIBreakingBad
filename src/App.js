...
class desafio extends Component {
    ...
    async onPagination() {
        ...
        for (let number = 1; number <= numPages + 1; number++) {
            this.state.items.push( <
                ...
                onClick = {
                    (e) => this.onChangePagination(number, e) } > { number } <
                /Pagination.Item>,
            );
        }
    }
    ...
    render() {
            return ( <
                ...
                a href = "/" >
                <
                img src = "https://yt3.ggpht.com/a/AGF-l79rcSfCz8vufHWw5DP4cTMYGvBpu135UlyfWw=s900-c-k-c0xffffffff-no-rj-mo"
                width = "20%"
                height = "20%"
                ...
                InputGroup.Text style = {
                    { backgroundColor: 'black' } }
                id = "basic-addon2" > < i style = {
                    { color: 'white' } }
                className = "material-icons" > search < /i></InputGroup.Text >
                <
                /InputGroup.Append> <
                /InputGroup> <
                /Form>
                ...
                Row style = {
                    { margin: '0.4em' } } > {
                    this.state.search && < p style = {
                        { color: 'grey' } } > Você pesquisou por "{this.state.search}" < /p>} <
                    /Row> <
                    Pagination style = {
                        { margin: '0.8rem' } }
                    className = "justify-content-md-center"
                    size = "sm" > { this.state.items } < /Pagination> <
...

                    {
                        this.state.charactersList.map((character) => {
                            ...
                            return (
                                ...
                                <
                                span style = {
                                    { fontSize: '0.8rem', marginTop: '0.4rem' } } > < i style = {
                                    { marginRight: '0.3rem' } }
                                className = "small material-icons" > star < /i>{character.birthday}</span > < br / >
                                <
                                span style = {
                                    { fontSize: '0.6rem' } } > { character.occupation.toString() } < /span> <
                                /Card.Text> <
                                /Card.Body> <
                                /Card>
